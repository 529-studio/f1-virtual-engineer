...
const LICENSE_URL = process.env.NEXT_PUBLIC_LICENSE_URL;
  ...
  return (
    <footer className="border-t border-border bg-surface">
      <div className="mx-auto flex max-w-6xl flex-col gap-3 px-6 py-6 sm:flex-row sm:items-center sm:justify-between">
        <p className="readout text-[0.7rem] uppercase tracking-(--track-wide) text-foreground-dim">
          © {year} Vu Thanh Dat (ヴ・タイン・ダット)
        </p>
        <nav className="flex items-center gap-5 text-[0.7rem] uppercase tracking-(--track-wide) text-foreground-dim">
          {LICENSE_URL && (
            <Link
              href={LICENSE_URL}
              target="_blank"
